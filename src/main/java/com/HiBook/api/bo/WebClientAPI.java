...
public class WebClientAPI {
	...
	// book 베스트셀러 api -10개
		public String bestSellerBookList() {
			//(listUrl + "?ttbkey=" + key + bestQuery + "&MaxResults=10&start=1&SearchTarget=Book&output=JS&Version=20131101").build();
			...
			String response = webClient.get()
					.uri(uriBuilder -> uriBuilder.path("/ttb/api/ItemList.aspx").queryParam("ttbkey", key)
							.queryParam("QueryType", QueryType).queryParam("MaxResults", MaxResults)
							.queryParam("start", start).queryParam("SearchTarget", SearchTarget)
							.queryParam("output", output).queryParam("Version", Version).build())
...
//			&Version=20131101
	...
	public String inquiryBookAPI(String isbn13) {
		...
		String response = webClient.get()
				.uri(uriBuilder -> uriBuilder.path("/ttb/api/ItemLookUp.aspx").queryParam("ttbkey", key)
						.queryParam("itemIdType", itemIdType)
						.queryParam("ItemId", ItemId)
						.queryParam("Cover", Cover)
						.queryParam("output", output)
						.queryParam("Version", Version)
						.queryParam("OptResult", OptResult).build())
				.retrieve() // 응답값을 받게 해주는 메소드
				.bodyToMono(String.class)// response body를 String 타입으로 받게 해줌
				.block(); // webclient는 기본적으로 비동기 방식인데 block메서드를 이용해 동기 방식 -없으면 String으로 못 바꿈.
...
//			&output=JS
//			&Version=20131101
	public String keywordBookAPI(String keyword) {
		...
		String response = webClient.get()
				.uri(uriBuilder -> uriBuilder.path("/ttb/api/ItemSearch.aspx").queryParam("ttbkey", key)
